...
for (const line of data.split('\n').splice(3)) {
	...
	if (line.trim() !== '') {
		...
		const newLine = splitLine.splice(12).slice(0, -24);
...
const learning_experience_total = Object.values(genders).reduce((a, b) => {
	if (a.length > 0) {
...
const result = { headers, genders, responses, learning_experience_total, ages };
